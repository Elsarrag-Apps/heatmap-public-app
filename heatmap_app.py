...
def run_urban_heat_risk(left_col, right_col, Map):
    ...
    if run_analysis:
        ...
        if location:
            ...
            fv = ndvi.subtract(ndvi_min).divide(ee.Number(ndvi_max).subtract(ndvi_min)).pow(2)
            ...
            try:
                ...
                st.warning("⚠️ Could not compute values.")
    ...
    with right_col:
        ...
        if "aoi" in st.session_state:
            try:
                ...
                st.warning("⚠️ AOI could not be shown.")
        ...
        try:
            ...
            st.warning("⚠️ LST layer could not be displayed.")
        ...
        try:
            ...
            st.warning("⚠️ UTFVI layer could not be displayed.")
...
# === Building Overheating Risk (placeholder) ===
def run_building_overheating_risk(left_col, right_col, Map):
    with left_col:
        ...
        st.write("📍 Building risk integration coming soon...")

    with right_col:
        st.markdown("### Building Risk Map (Coming Soon)")
        Map.to_streamlit(width=700, height=500, scrolling=True, add_layer_control=True)
